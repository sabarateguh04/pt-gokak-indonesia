...
- `pt_kapuk_teknisi_area`: 1 teknisi boleh di-assign ke **beberapa area** dari master `pt_kapuk_area` (dikelola dari halaman Teknisi). **Sifatnya organisasi/informasi** ("teknisi ini seharusnya kerja di area mana") — BUKAN yang nentuin `in_area` di atas, itu tetap union semua area `is_primary` apapun teknisinya. Ini JUGA yang jadi sumber pilihan dropdown "Area/Lokasi Kerja" pas bikin tiket (lihat bagian 6a).
...
- **Cara hitung** (`helpers/kehadiran.js`) — GAK ada tabel sesi terpisah, dihitung dari **gap antar ping GPS**: ping dikirim tiap 2 detik selama online; gap wajar (≤ 30 detik — pakai lantai minimum, bukan cuma "5× interval", biar tetep longgar buat jeda network/GPS sebentar) dianggap online penuh & diklasifikasi in-area/out-area dari kolom `in_area` ping tersebut; gap yang jauh lebih lama (device mati/tab ditutup) dianggap disconnect dan dipotong ke 30 detik.
  ...
  - Endpoint `/api/kpi/ringkasan` (tabel SEMUA karyawan sekaligus) pakai **pendekatan cepat** `COUNT(ping) × 2 detik` di level SQL supaya tetap ringan di skala 500–1000 karyawan — sedikit kurang presisi di ekor sesi, tapi valid karena ping memang terkirim tiap 2 detik selama online. ⚠️ Interval 2 detik = volume baris GPS ~15× lebih banyak dibanding kalau 30 detik — lihat catatan skala di bagian 15.
...
- **Pin nama di tiap area** — label `📍 Nama Area` ngambang di titik tengah tiap poligon, biar keliatan area mana yang mana dari atas tanpa perlu klik satu-satu.
...
- **Interval GPS 2 detik** (`PING_INTERVAL_SECONDS` di `helpers/kehadiran.js` + `public/js/teknisi-tracker.js`) berarti tabel `pt_kapuk_teknisi_lokasi` numpuk ~15× lebih cepat dibanding kalau 30 detik (1 karyawan online 8 jam ≈ 14.400 baris/hari). Belum ada job pembersihan data lama (retensi) -- kalau tabelnya mulai berat, ini yang paling perlu ditambah duluan (mis. hapus/archive ping yang lebih tua dari N bulan, riwayat harian yang udah ke-agregat gak butuh raw ping-nya lagi).
- `/api/kpi/ringkasan` masih query mentah tiap request (gak ada tabel rollup harian) — cukup buat skala saat ini, tapi kalau volume ping makin gede ke depannya (apalagi sekarang 2 detik), pertimbangkan tabel `pt_kapuk_kehadiran_harian` yang di-update berkala.
